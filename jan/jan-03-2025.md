...
By looking around, i found this article for javafx and css.

Link: https://docs.oracle.com/javafx/2/css_tutorial/jfxpub-css_tutorial.htm

With this link, it simplifies css. By removing and adding css files to scene
we can change the entire app.

I created 7 Buttons that replace the stylesheet in scene. this auto refreshes
so it doesn't manually have to be reloaded.
```java
Scene scene = new Scene(new VBox()); // scene

// remove all css files
scene.getStylesheets().removeAll(scene.getStylesheets());

// add the sole css file.
scene.getStylesheets().add(getClass().getResource("/com/aadarsh/testproject/changablecss/test_6.css").toExternalForm());
```
